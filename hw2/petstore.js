...
function mostPopularDays(week) {
    ...
    var mostPopular = week.filter(day => {
        return day.traffic === mostTraffic
    });

    if (mostPopular.length == 1) {
        return mostPopular[0].name;
    }

    mostPopular = mostPopular.map(popular => {
        return popular.name;
    })
...
function createAnimalObjects(names, types, breeds) {
    if (names === null || types === null || breeds === null || 
        ...
        || types.length !== names.length || breeds.length !== names.length) {
            return [];
        }

    return names.map((name, idx) => {
        return new Animal(name, types[idx], breeds[idx]);
    })
}
